...
function EditProductScreen() {
    ...
    const loadUserData = async () => {
        ...
        if (userData) {
            ...
            try {
                const response = await fetch(`https://aqtas.ru/customers/${userData.userId}`);
                if (response.ok) {
    ...
    const uploadProductImages = async () => {
        ...
        try {
            const response = await fetch(`https://aqtas.ru/updateProductImage/${userData.userId}/${productData.id}`, {
                method: 'POST',
                body: formData,
                headers: {
                                ...
                                {productImages.map((image, index) => (
                                    <View style={styles.photo} key={index}>
                                        <Image
                                            ...
                                            source={{ uri: `https://aqtas.ru/images/imageProducts/${image}` }}
                                        />
                                    </View>
                                ))}
